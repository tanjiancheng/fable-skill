...
description: >
  ...
  parallel delegation via the Task/Agent tool, a failable verification check at
  each stage, and a skeptical self-review before delivery. Also trigger when
  the user explicitly asks ("do this thoroughly", "be systematic", "deep work
  mode") OR when the task objectively spans multiple files, multiple sources,
  or multiple sessions. Do NOT trigger on ordinary multi-step requests that a
  direct attempt handles fine. Operational guardrails (verify-before-flag,
  warning batching, sed safety) are inlined in the Operational rules section
  below and apply on every model and every task, regardless of whether this
  loop runs.
---
...
The loop's value inverts with model strength. Identify which model is executing (Claude
Code exposes this; if unsure, assume Sonnet-tier) and apply the right intensity:
...
**2. Delegate independent work (via the Task/Agent tool)**
Claude Code exposes subagent tooling (the Task tool, with an optional `model` parameter:
`"opus"`, `"sonnet"`, `"haiku"`, or `"inherit"`). If stage N and stage M don't depend on
each other, spawn them concurrently. Route by cost: research/bulk-processing sub-parts
can run on `sonnet` or `haiku`; synthesis-heavy sub-parts stay on the strongest available
model. Each subagent should be briefed with: its specific task, what it should produce,
where to save outputs, any relevant context from prior stages, and the Operational rules
below verbatim — a spawned subagent cannot see this skill.
...
If the current surface has no subagent tooling, run the stages sequentially and proceed
to step 3.
